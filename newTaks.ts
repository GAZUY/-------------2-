...
function CombiningArrays_1 (arr10: any [],arr5: any [],) {
    let arr: any =[]
    for (let el of arr5) {
        if (arr10.includes(el)) arr.push(el)
    }
    // for (let i = 0; i < arr10.length; i++) {
    //   if (arr5.includes(arr10[i])) {
    //     arr.push(arr10[i]);
  ...
  function CombiningArrays_2 (arr10: any [],arr5: any [],) {
    let arr: any =[]
    for (let i = 0; i < arr10.length; i++) {
      if (!arr5.includes(arr10[i])) {
        arr.push(arr10[i]);
  ...
  const fruitList = ['lemon', 'watermelon', 'quince', 'mango', 'tangerine', 'melon', ] 
  document.write (`<p>${(fruitList.sort((a, b) => a.localeCompare(b)).join(', '))}</p>`)
  document.write (`<hr>`)
  ...
  function lookingForFruit (f: string) {
